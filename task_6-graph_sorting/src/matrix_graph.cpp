...
MatrixGraph::MatrixGraph(MatrixGraph &other) : numberOfVertices(other.numberOfVertices) {
    matrix = new int *[other.numberOfVertices];
...
int MatrixGraph::vertexOutDegree(int idx) {
    int deg = 0;
    for (int i = 0; i < numberOfVertices; i++) {
        if (matrix[idx][i] > 0)
            deg++;
...
int MatrixGraph::vertexInDegree(int idx) {
    int deg = 0;
    for (int i = 0; i < numberOfVertices; i++) {
        if (matrix[i][idx] > 0)
            deg++;
...
bool MatrixGraph::isPresent(int idx) {
    for (int i = 0; i < numberOfVertices; i++) {
        if (matrix[idx][i] > 0 || matrix[i][idx] > 0)
            return true;
    }
    return false;
}

int MatrixGraph::getRealSize() {
    int size = 0;
    for (int i = 0; i < numberOfVertices; i++ ) {
        if (isPresent(i))
            size++;
...
std::vector<int> MatrixGraph::topologicalSort() {
    std::queue<int> doomersQueue;
    int neighboursCount[numberOfVertices];
    std::vector<int> result;

    for (int i = 0; i < numberOfVertices; i++) {
        neighboursCount[i] = vertexInDegree(i);
        if (isPresent(i) && vertexInDegree(i) == 0) {
            doomersQueue.push(i);
        }
    }
    ...
    while (!doomersQueue.empty()) {
        ...
        doomersQueue.pop();
        for (int neighbour : getNeighbourIndices(currentVertex)) {
            neighboursCount[neighbour]--;
            if (neighboursCount[neighbour] == 0) {
                doomersQueue.push(neighbour);
            }
        }
    }
    return result;
